...
function updateProfile(user) {
    ...
    if (newName !== "" || profilePictureInput.files.length > 0) {
        ...
        if (profilePictureInput.files.length > 0) {
            ...
            // Display the selected profile picture immediately
            profilePicture.src = URL.createObjectURL(newProfilePicture);

            // Add a promise to resolve when the profile picture update is complete
            updatePromises.push(
                new Promise((resolve, reject) => {
                    // Add event listeners to the "Save" and "Discard" buttons
                                ...
                                uploadTask.snapshot.ref
                                    ...
                                    .then((downloadURL) => {
                                        ...
                                        user
                                            ...
                                            .then(() => {
                                                // Update successful, hide the loading spinner and display success message
                    ...
                    discardButton.addEventListener("click", () => {
                        // Discard the selected profile picture
                        profilePicture.src = user.photoURL || "/img/default-profile.jpg";
                        resolve();
                    });
                })
            );
        ...
        Promise.all(updatePromises)
            .then(() => {
                // All updates are successful, hide the loading spinner
                loadingSpinner.classList.add("hidden");
            })
            .catch((error) => {
